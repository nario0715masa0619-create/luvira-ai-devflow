...
class V3Transaction:
    ...
    def record_publication_recovery(self, task: V3Task, record: ExecutionRecord) -> None:
        """Persist a classified publication failure while retaining the artifact."""
        if (task.status is not V3Status.ARTIFACT_VERIFIED or task.execution is not record
                or not record.failure_code):
            raise V3TransactionError("publication_recovery_invalid")
        task_ref = self.tasks.document(task.task_id)
        def write(transaction: Any) -> None:
            snapshot = task_ref.get(transaction=transaction)
            if not snapshot.exists:
