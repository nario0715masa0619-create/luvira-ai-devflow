...
class ExecutionRecordStoreTest(unittest.TestCase):
    def test_record_payload_is_allow_listed_and_does_not_add_arbitrary_data(self):
        ...
        self.assertEqual(set(payload), {"execution_id", "task_id", "spec_hash", "attempt", "status", "provider", "external_operation_id", "launch_operation_id", "failure_code", "stream_events", "last_progress_at", "implementation_claimed_at", "publication_url", "revision"})
