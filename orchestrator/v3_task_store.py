...
def task_payload(task: V3Task) -> dict[str, Any]:
    ...
    if task.execution is not None:
        payload["execution"] = {
            ...
            "launch_operation_id": task.execution.launch_operation_id,
...
def task_from_payload(value: dict[str, Any]) -> V3Task:
    try:
        ...
        execution = None if execution_value is None else ExecutionRecord(
            ...
            launch_operation_id=execution_value.get("launch_operation_id"),
...
class FirestoreV3TaskStore:
    ...
    def running(self):
        ...
        return self._with_status([V3Status.EXECUTION_RUNNING.value, V3Status.WORKER_LAUNCH_ACCEPTED.value])
